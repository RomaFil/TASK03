...
#ifdef CONFIG_BTRFS_DEBUG
  WARN_RATELIMIT(
      1, "qgroup %llu %s reserved space underflow, have %llu to free %llu",
      qgroup->qgroupid, qgroup_rsv_type_str(type), qgroup->rsv.values[type],
      num_bytes);
#endif
...
static void btrfs_qgroup_update_old_refcnt(struct btrfs_qgroup* qg,
                                           u64 seq,
                                           int mod) {
...
static void btrfs_qgroup_update_new_refcnt(struct btrfs_qgroup* qg,
                                           u64 seq,
                                           int mod) {
...
static inline u64 btrfs_qgroup_get_old_refcnt(const struct btrfs_qgroup* qg,
                                              u64 seq) {
  if (qg->old_refcnt < seq)
    return 0;
...
static inline u64 btrfs_qgroup_get_new_refcnt(const struct btrfs_qgroup* qg,
                                              u64 seq) {
  if (qg->new_refcnt < seq)
    return 0;
...
static int btrfs_qgroup_qgroupid_key_cmp(const void* key,
...
static struct btrfs_qgroup* find_qgroup_rb(const struct btrfs_fs_info* fs_info,
  ...
  node =
      rb_find(&qgroupid, &fs_info->qgroup_tree, btrfs_qgroup_qgroupid_key_cmp);
  return rb_entry_safe(node, struct btrfs_qgroup, node);
}

static int btrfs_qgroup_qgroupid_cmp(struct rb_node* new,
                                     const struct rb_node* existing) {
...
static struct btrfs_qgroup* add_qgroup_rb(struct btrfs_fs_info* fs_info,
  ...
  node = rb_find_add(&prealloc->node, &fs_info->qgroup_tree,
                     btrfs_qgroup_qgroupid_cmp);
  if (node) {
    kfree(prealloc);
...
static void __del_qgroup_rb(struct btrfs_qgroup* qgroup) {
  ...
  while (!list_empty(&qgroup->groups)) {
    list =
        list_first_entry(&qgroup->groups, struct btrfs_qgroup_list, next_group);
  ...
  while (!list_empty(&qgroup->members)) {
    list = list_first_entry(&qgroup->members, struct btrfs_qgroup_list,
                            next_member);
    list_del(&list->next_group);
    list_del(&list->next_member);
    kfree(list);
...
static bool squota_check_parent_usage(struct btrfs_fs_info* fs_info,
                                      struct btrfs_qgroup* parent) {
  u64 excl_sum = 0;
  u64 rfer_sum = 0;
  u64 excl_cmpr_sum = 0;
  ...
  mismatch = (parent->excl != excl_sum || parent->rfer != rfer_sum ||
              parent->excl_cmpr != excl_cmpr_sum ||
              parent->rfer_cmpr != rfer_cmpr_sum);

  WARN(mismatch,
       "parent squota qgroup " BTRFS_QGROUP_FMT
       " has mismatched usage from its %d members. "
       "%llu %llu %llu %llu vs %llu %llu %llu %llu\n",
       BTRFS_QGROUP_FMT_VALUE(parent), nr_members, parent->excl, parent->rfer,
       parent->excl_cmpr, parent->rfer_cmpr, excl_sum, rfer_sum, excl_cmpr_sum,
       rfer_cmpr_sum);
  return mismatch;
}

__printf(2,
         3) static void qgroup_mark_inconsistent(struct btrfs_fs_info* fs_info,
                                                 const char* fmt,
                                                 ...) {
...
int btrfs_read_qgroup_config(struct btrfs_fs_info* fs_info) {
  ...
  while (1) {
    ...
    if (found_key.type == BTRFS_QGROUP_STATUS_KEY) {
      ...
      if (btrfs_qgroup_status_version(l, ptr) != BTRFS_QGROUP_STATUS_VERSION) {
        btrfs_err(fs_info, "old qgroup version, quota disabled");
        goto out;
      }
    ...
    switch (found_key.type) {
      case BTRFS_QGROUP_INFO_KEY: {
        ...
        ptr = btrfs_item_ptr(l, slot, struct btrfs_qgroup_info_item);
      ...
      case BTRFS_QGROUP_LIMIT_KEY: {
        ...
        ptr = btrfs_item_ptr(l, slot, struct btrfs_qgroup_limit_item);
  ...
  while (1) {
    ...
    ret = add_relation_rb(fs_info, list, found_key.objectid, found_key.offset);
    list = NULL;
    if (ret == -ENOENT) {
  ...
  for (node = rb_first(&fs_info->qgroup_tree); node; node = rb_next(node)) {
    ...
    for (i = 0; i < BTRFS_QGROUP_RSV_LAST; i++) {
      if (qgroup->rsv.values[i]) {
        ret = true;
        btrfs_warn(fs_info,
                   "qgroup " BTRFS_QGROUP_FMT
                   " has unreleased space, type %d rsv %llu",
                   BTRFS_QGROUP_FMT_VALUE(qgroup), i, qgroup->rsv.values[i]);
      }
    }
  }
  return ret;
...
static int add_qgroup_item(struct btrfs_trans_handle* trans,
  ...
  leaf = path->nodes[0];
  qgroup_info =
      btrfs_item_ptr(leaf, path->slots[0], struct btrfs_qgroup_info_item);
  ...
  leaf = path->nodes[0];
  qgroup_limit =
      btrfs_item_ptr(leaf, path->slots[0], struct btrfs_qgroup_limit_item);
...
static int update_qgroup_status_item(struct btrfs_trans_handle* trans) {
  ...
  btrfs_set_qgroup_status_flags(
      l, ptr, fs_info->qgroup_flags & BTRFS_QGROUP_STATUS_FLAGS_MASK);
...
int btrfs_quota_enable(struct btrfs_fs_info* fs_info,
  ...
  if (btrfs_fs_incompat(fs_info, EXTENT_TREE_V2)) {
    btrfs_err(fs_info, "qgroups are currently unsupported in extent tree v2");
    return -EINVAL;
  }
  ...
  ret = btrfs_insert_empty_item(trans, quota_root, path, &key, sizeof(*ptr));
  if (unlikely(ret)) {
    btrfs_abort_transaction(trans, ret);
    goto out_free_path;
  }
  ...
  leaf = path->nodes[0];
  ptr = btrfs_item_ptr(leaf, path->slots[0], struct btrfs_qgroup_status_item);
  ...
  btrfs_set_qgroup_status_flags(
      leaf, ptr, fs_info->qgroup_flags & BTRFS_QGROUP_STATUS_FLAGS_MASK);
  ...
  while (1) {
    ...
    if (found_key.type == BTRFS_ROOT_REF_KEY) {
      ...
      ret = add_qgroup_item(trans, quota_root, found_key.offset);
      if (unlikely(ret)) {
        btrfs_abort_transaction(trans, ret);
        goto out_free_path;
      }
      ...
      ret = btrfs_search_slot_for_read(tree_root, &found_key, path, 1, 0);
      if (unlikely(ret < 0)) {
        btrfs_abort_transaction(trans, ret);
        goto out_free_path;
      }
...
static bool can_delete_squota_subvol_qgroup(struct btrfs_fs_info* fs_info,
  ...
  return !(qgroup->rfer || qgroup->excl || qgroup->rfer_cmpr ||
           qgroup->excl_cmpr);
}
...
int btrfs_remove_qgroup(struct btrfs_trans_handle* trans, u64 qgroupid) {
  ...
  if (unlikely(qgroup->rsv.values[BTRFS_QGROUP_RSV_DATA] ||
    ...
    btrfs_warn_rl(fs_info,
                  "to be deleted qgroup " BTRFS_QGROUP_FMT
                  " has non-zero numbers, data %llu meta prealloc %llu meta "
                  "pertrans %llu",
  ...
  if (btrfs_qgroup_mode(fs_info) == BTRFS_QGROUP_MODE_FULL &&
      !(fs_info->qgroup_flags & BTRFS_QGROUP_STATUS_FLAG_INCONSISTENT)) {
    if (unlikely(qgroup->rfer || qgroup->excl || qgroup->rfer_cmpr ||
                 qgroup->excl_cmpr)) {
      DEBUG_WARN();
      qgroup_mark_inconsistent(fs_info,
                               "to be deleted qgroup " BTRFS_QGROUP_FMT
                               " has non-zero numbers, rfer %llu rfer_cmpr "
                               "%llu excl %llu excl_cmpr %llu",
                               BTRFS_QGROUP_FMT_VALUE(qgroup), qgroup->rfer,
                               qgroup->rfer_cmpr, qgroup->excl,
                               qgroup->excl_cmpr);
    }
  }
...
int btrfs_qgroup_trace_extent_nolock(
  ...
  if (bytenr >= MAX_LFS_FILESIZE) {
    btrfs_err_rl(fs_info,
                 "qgroup record for extent at %llu is beyond 32bit page cache "
                 "and xarray index limit",
                 bytenr);
...
int btrfs_qgroup_trace_extent_post(struct btrfs_trans_handle* trans,
  ...
  if (ret < 0) {
    qgroup_mark_inconsistent(
        fs_info, "error accounting new delayed refs extent: %d", ret);
    return 0;
  }
...
static int qgroup_trace_new_subtree_blocks(struct btrfs_trans_handle* trans,
  ...
  if (unlikely(cur_level < 0 || cur_level >= BTRFS_MAX_LEVEL - 1 ||
    ...
    btrfs_err_rl(fs_info, "%s: bad levels, cur_level=%d root_level=%d",
  ...
  ret = qgroup_trace_extent_swap(trans, src_eb, dst_path, cur_level, root_level,
                                 trace_leaf);
  if (ret < 0)
    goto cleanup;
  ...
  if (cur_level > 0) {
    ...
    for (i = 0; i < btrfs_header_nritems(eb); i++) {
      ...
      ret = qgroup_trace_new_subtree_blocks(trans, src_eb, dst_path,
                                            cur_level - 1, root_level,
...
static int qgroup_trace_subtree_swap(struct btrfs_trans_handle* trans,
  ...
  if (unlikely(btrfs_header_generation(src_eb) >
               btrfs_header_generation(dst_eb))) {
    btrfs_err_rl(fs_info, "%s: bad parameter order, src_gen=%llu dst_gen=%llu",
                 __func__, btrfs_header_generation(src_eb),
                 btrfs_header_generation(dst_eb));
    return -EUCLEAN;
  }

  if (unlikely(!extent_buffer_uptodate(src_eb) ||
  ...
  ret = qgroup_trace_new_subtree_blocks(trans, src_eb, dst_path, level, level,
                                        last_snapshot, trace_leaf);
  if (ret < 0)
    goto out;
  ret = 0;
...
int btrfs_qgroup_trace_subtree(struct btrfs_trans_handle* trans,
  ...
  if (!extent_buffer_uptodate(root_eb)) {
    struct btrfs_tree_parent_check check = {.transid = root_gen,
                                            .level = root_level};
...
walk_down:
  ...
  while (level >= 0) {
    if (path->nodes[level] == NULL) {
      ...
      ret = btrfs_qgroup_trace_extent(trans, child_bytenr, fs_info->nodesize);
      if (ret)
        return ret;
    }
...
static void qgroup_update_counters(struct btrfs_fs_info* fs_info,
  ...
  list_for_each_entry(qg, qgroups, nested_iterator) {
    ...
    if (cur_old_count == nr_old_roots && cur_new_count < nr_new_roots) {
      /* Exclusive -> shared */
      if (cur_old_count != 0) {
        qg->excl -= num_bytes;
...
int btrfs_qgroup_account_extent(struct btrfs_trans_handle* trans,
  ...
  trace_btrfs_qgroup_account_extent(fs_info, trans->transid, bytenr, num_bytes,
...
int btrfs_qgroup_account_extents(struct btrfs_trans_handle* trans) {
  ...
  xa_for_each(&delayed_refs->dirty_extents, index, record) {
    ...
    if (!ret &&
      ...
      ret = btrfs_qgroup_account_extent(trans, bytenr, record->num_bytes,
                                        record->old_roots, new_roots);
      record->old_roots = NULL;
      new_roots = NULL;
    }
    /* Free the reserved data space */
    btrfs_qgroup_free_refroot(fs_info, record->data_rsv_refroot,
                              record->data_rsv, BTRFS_QGROUP_RSV_DATA);
  cleanup:
...
int btrfs_run_qgroups(struct btrfs_trans_handle* trans) {
  ...
  while (!list_empty(&fs_info->dirty_qgroups)) {
    struct btrfs_qgroup* qgroup;
    qgroup =
        list_first_entry(&fs_info->dirty_qgroups, struct btrfs_qgroup, dirty);
    list_del_init(&qgroup->dirty);
    spin_unlock(&fs_info->qgroup_lock);
    ret = update_qgroup_info_item(trans, qgroup);
    if (ret)
      qgroup_mark_inconsistent(fs_info, "qgroup info item update error %d",
                               ret);
    ret = update_qgroup_limit_item(trans, qgroup);
    if (ret)
      qgroup_mark_inconsistent(fs_info, "qgroup limit item update error %d",
                               ret);
    spin_lock(&fs_info->qgroup_lock);
  }
  ...
  if (ret)
    qgroup_mark_inconsistent(fs_info, "qgroup status item update error %d",
                             ret);
...
static int qgroup_auto_inherit(struct btrfs_fs_info* fs_info,
  ...
  list_for_each_entry(qg_list, &inode_qg->groups, next_group) qgids[i++] =
      qg_list->group->qgroupid;
...
int btrfs_qgroup_inherit(struct btrfs_trans_handle* trans,
  ...
  if (inherit) {
    ...
    for (int i = 0; i < nums; i++) {
       ...
       * them later.
       */
      if (!srcgroup || ((srcgroup->qgroupid >> 48) <= (objectid >> 48)))
        *i_qgroups = 0ULL;
  ...
  if (inherit) {
    ...
    qlist_prealloc =
        kzalloc_objs(struct btrfs_qgroup_list*, inherit->num_qgroups, GFP_NOFS);
    if (!qlist_prealloc) {
      ret = -ENOMEM;
      goto out;
    }
    for (int i = 0; i < inherit->num_qgroups; i++) {
      qlist_prealloc[i] = kzalloc_obj(struct btrfs_qgroup_list, GFP_NOFS);
      if (!qlist_prealloc[i]) {
        ret = -ENOMEM;
        goto out;
      }
...
static int qgroup_reserve(struct btrfs_root* root,
                          u64 num_bytes,
                          bool enforce,
                          enum btrfs_qgroup_rsv_type type) {
  struct btrfs_qgroup* qgroup;
...
static int qgroup_rescan_leaf(struct btrfs_trans_handle* trans,
  ...
  mutex_lock(&fs_info->qgroup_rescan_lock);
  extent_root =
      btrfs_extent_root(fs_info, fs_info->qgroup_rescan_progress.objectid);
  ...
  for (; slot < btrfs_header_nritems(scratch_leaf); ++slot) {
    ...
    ret = btrfs_qgroup_account_extent(trans, found.objectid, num_bytes, NULL,
                                      ctx.roots);
    if (ret < 0)
      goto out;
  }
...
static void btrfs_qgroup_rescan_worker(struct btrfs_work* work) {
  struct btrfs_fs_info* fs_info =
      container_of(work, struct btrfs_fs_info, qgroup_rescan_work);
  struct btrfs_path* path;
  struct btrfs_trans_handle* trans = NULL;
  int ret = 0;
  bool stopped = false;
...
out:
  ...
  if (did_leaf_rescans) {
    ...
    if (IS_ERR(trans)) {
      ret = PTR_ERR(trans);
      trans = NULL;
      btrfs_err(fs_info, "fail to start transaction for status update: %pe",
                ERR_PTR(ret));
    }
  } else {
    trans = NULL;
...
static int qgroup_rescan_init(struct btrfs_fs_info* fs_info,
  ...
  btrfs_init_work(&fs_info->qgroup_rescan_work, btrfs_qgroup_rescan_worker,
                  NULL);
  return 0;
}
...
static int qgroup_unreserve_range(struct btrfs_inode* inode,
  ...
  if (entry->val > start && rb_prev(&entry->rb_node))
    entry = rb_entry(rb_prev(&entry->rb_node), struct ulist_node, rb_node);
 ...
 * run_delalloc_range(). In theory, we should only flush nodatacow inodes, but
 * it's not yet possible, so we need to flush the whole root.
 *
...
static int qgroup_reserve_data(struct btrfs_inode* inode,
                               struct extent_changeset** reserved_ret,
                               u64 start,
                               u64 len) {
  ...
  ret = btrfs_set_record_extent_bits(&inode->io_tree, start, start + len - 1,
                                     EXTENT_QGROUP_RESERVED, reserved);

  /* Newly reserved space */
  to_reserve = reserved->bytes_changed - orig_reserved;
  trace_btrfs_qgroup_reserve_data(&inode->vfs_inode, start, len, to_reserve,
                                  QGROUP_RESERVE);
  if (ret < 0)
    goto out;
...
static int qgroup_free_reserved_data(struct btrfs_inode* inode,
  ...
  while ((unode = ulist_next(&reserved->range_changed, &uiter))) {
    ...
    free_len =
        min(aligned_start + aligned_len, range_start + range_len) - free_start;
    /*
...
static int __btrfs_qgroup_release_data(struct btrfs_inode* inode,
  ...
  if (btrfs_qgroup_mode(inode->root->fs_info) == BTRFS_QGROUP_MODE_DISABLED) {
    return btrfs_clear_record_extent_bits(
        &inode->io_tree, start, start + len - 1, EXTENT_QGROUP_RESERVED, NULL);
  }
  ...
  if (free)
    btrfs_qgroup_free_refroot(inode->root->fs_info, btrfs_root_id(inode->root),
 ...
 * NOTE: This function may sleep for memory allocation.
 */
int btrfs_qgroup_release_data(struct btrfs_inode* inode,
                              u64 start,
                              u64 len,
                              u64* released) {
...
static void add_root_meta_rsv(struct btrfs_root* root,
                              int num_bytes,
                              enum btrfs_qgroup_rsv_type type) {
  if (type != BTRFS_QGROUP_RSV_META_PREALLOC &&
...
static int sub_root_meta_rsv(struct btrfs_root* root,
                             int num_bytes,
                             enum btrfs_qgroup_rsv_type type) {
  if (type != BTRFS_QGROUP_RSV_META_PREALLOC &&
      type != BTRFS_QGROUP_RSV_META_PERTRANS)
  ...
  spin_lock(&root->qgroup_meta_rsv_lock);
  if (type == BTRFS_QGROUP_RSV_META_PREALLOC) {
    num_bytes = min_t(u64, root->qgroup_meta_rsv_prealloc, num_bytes);
    root->qgroup_meta_rsv_prealloc -= num_bytes;
  } else {
    num_bytes = min_t(u64, root->qgroup_meta_rsv_pertrans, num_bytes);
    root->qgroup_meta_rsv_pertrans -= num_bytes;
  }
  spin_unlock(&root->qgroup_meta_rsv_lock);
  return num_bytes;
...
static int btrfs_qgroup_reserve_meta(struct btrfs_root* root,
                                     int num_bytes,
                                     enum btrfs_qgroup_rsv_type type,
                                     bool enforce) {
  struct btrfs_fs_info* fs_info = root->fs_info;
...
int btrfs_qgroup_reserve_meta_prealloc(struct btrfs_root* root,
                                       int num_bytes,
                                       bool enforce,
                                       bool noflush) {
  int ret;
...
void btrfs_qgroup_free_meta_prealloc(struct btrfs_root* root, int num_bytes) {
   ...
   */
  num_bytes =
      sub_root_meta_rsv(root, num_bytes, BTRFS_QGROUP_RSV_META_PREALLOC);
  BUG_ON(num_bytes != round_down(num_bytes, fs_info->nodesize));
...
void btrfs_qgroup_clean_swapped_blocks(struct btrfs_root* root) {
  ...
  for (i = 0; i < BTRFS_MAX_LEVEL; i++) {
    ...
    rbtree_postorder_for_each_entry_safe(entry, next, cur_root, node)
        kfree(entry);
    swapped_blocks->blocks[i] = RB_ROOT;
  }
  swapped_blocks->swapped = false;
...
static int qgroup_swapped_block_bytenr_key_cmp(const void* key,
                                               const struct rb_node* node) {
...
static int qgroup_swapped_block_bytenr_cmp(struct rb_node* new,
                                           const struct rb_node* existing) {
  const struct btrfs_qgroup_swapped_block* new_block =
      rb_entry(new, struct btrfs_qgroup_swapped_block, node);

  return qgroup_swapped_block_bytenr_key_cmp(&new_block->subvol_bytenr,
                                             existing);
}
...
int btrfs_qgroup_add_swapped_blocks(struct btrfs_root* subvol_root,
                                    ...
                                    struct extent_buffer* subvol_parent,
                                    int subvol_slot,
                                    struct extent_buffer* reloc_parent,
                                    int reloc_slot,
                                    u64 last_snapshot) {
  ...
  if (unlikely(btrfs_node_ptr_generation(subvol_parent, subvol_slot) >
    ...
    btrfs_err_rl(
        fs_info, "%s: bad parameter order, subvol_gen=%llu reloc_gen=%llu",
        __func__, btrfs_node_ptr_generation(subvol_parent, subvol_slot),
        btrfs_node_ptr_generation(reloc_parent, reloc_slot));
    return -EUCLEAN;
  }
...
int btrfs_record_squota_delta(struct btrfs_fs_info* fs_info,
  ...
  list_for_each_entry(qg, &qgroup_list, iterator) {
    ...
    if (WARN_ON_ONCE(sign < 0 && qg->excl < num_bytes)) {
      btrfs_warn(fs_info,
                 "squota underflow qg " BTRFS_QGROUP_FMT
                 " excl %llu num_bytes %llu",
                 BTRFS_QGROUP_FMT_VALUE(qg), qg->excl, num_bytes);
      qg->excl = 0;
      qg->rfer = 0;
    } else {
