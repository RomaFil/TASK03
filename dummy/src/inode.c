...
static __u32 ext4_inode_csum(struct inode* inode,
  ...
  offset += csum_size;
  csum =
      ext4_chksum(csum, (__u8*)raw + offset, EXT4_GOOD_OLD_INODE_SIZE - offset);
...
static int ext4_inode_csum_verify(struct inode* inode,
                                  struct ext4_inode* raw,
                                  struct ext4_inode_info* ei) {
...
static inline int ext4_begin_ordered_truncate(struct inode* inode,
  ...
  if (!jinode)
    return 0;
  return jbd2_journal_begin_ordered_truncate(EXT4_JOURNAL(inode), jinode,
                                             new_size);
}
...
int ext4_inode_is_fast_symlink(struct inode* inode) {
  if (!ext4_has_feature_ea_inode(inode->i_sb)) {
    int ea_blocks =
        EXT4_I(inode)->i_file_acl ? EXT4_CLUSTER_SIZE(inode->i_sb) >> 9 : 0;
...
void ext4_evict_inode(struct inode* inode) {
  ...
  if (inode->i_blocks) {
    ...
    if (err) {
      ext4_error_err(inode->i_sb, -err, "couldn't truncate inode %llu (err %d)",
                     inode->i_ino, err);
      goto stop_handle;
    }
  }
...
void ext4_da_update_reserve_space(struct inode* inode,
   ...
   */
  if ((ei->i_reserved_data_blocks == 0) && !inode_is_open_for_write(inode))
    ext4_discard_preallocations(inode);
}

static int __check_block_validity(struct inode* inode,
                                  const char* func,
                                  unsigned int line,
...
int ext4_issue_zeroout(struct inode* inode,
                       ext4_lblk_t lblk,
                       ext4_fsblk_t pblk,
                       ext4_lblk_t len) {
  int ret;
  ...
  if (IS_ENCRYPTED(inode) && S_ISREG(inode->i_mode))
    return fscrypt_zeroout_range(inode, (loff_t)lblk << inode->i_blkbits,
...
void ext4_check_map_extents_env(struct inode* inode) {
  ...
  if (!S_ISREG(inode->i_mode) || IS_NOQUOTA(inode) || IS_VERITY(inode) ||
...
static void ext4_map_blocks_es_recheck(handle_t* handle,
  ...
  if (es_map->m_lblk != map->m_lblk || es_map->m_flags != map->m_flags ||
    ...
    printk(
        "ES cache assertion failed for inode: %llu "
        "es_cached ex [%d/%d/%llu/%x] != "
        "found ex [%d/%d/%llu/%x] retval %d flags %x\n",
        inode->i_ino, es_map->m_lblk, es_map->m_len, es_map->m_pblk,
        es_map->m_flags, map->m_lblk, map->m_len, map->m_pblk, map->m_flags,
        retval, flags);
  }
}
...
static int ext4_map_query_blocks_next_in_leaf(handle_t* handle,
  ...
  status2 = map2.m_flags & EXT4_MAP_UNWRITTEN ? EXTENT_STATUS_UNWRITTEN
                                              : EXTENT_STATUS_WRITTEN;
   ...
   */
  if (map->m_pblk + map->m_len == map2.m_pblk && status == status2) {
    ext4_es_cache_extent(inode, map->m_lblk, map->m_len + map2.m_len,
                         map->m_pblk, status);
...
int ext4_map_query_blocks(handle_t* handle,
                          struct inode* inode,
                          struct ext4_map_blocks* map,
                          int flags) {
  unsigned int status;
  ...
  if (!(map->m_flags & EXT4_MAP_QUERY_LAST_IN_LEAF) ||
    ...
    retval = ext4_map_query_blocks_next_in_leaf(handle, inode, map, orig_mlen);
  }
out:
...
int ext4_map_create_blocks(handle_t* handle,
                           struct inode* inode,
                           struct ext4_map_blocks* map,
                           int flags) {
  unsigned int status;
...
int ext4_map_blocks(handle_t* handle,
  ...
  if (ext4_es_lookup_extent(inode, map->m_lblk, NULL, &es, &map->m_seq)) {
    if (ext4_es_is_written(&es) || ext4_es_is_unwritten(&es)) {
      ...
      map->m_flags |=
          ext4_es_is_written(&es) ? EXT4_MAP_MAPPED : EXT4_MAP_UNWRITTEN;
...
static int _ext4_get_block(struct inode* inode,
                           sector_t iblock,
...
struct buffer_head* ext4_getblk(handle_t* handle,
  ...
  if (map.m_flags & EXT4_MAP_NEW) {
    ...
    err =
        ext4_journal_get_create_access(handle, inode->i_sb, bh, EXT4_JTR_NONE);
    if (unlikely(err)) {
      unlock_buffer(bh);
      goto errout;
    }
...
int do_journal_get_write_access(handle_t* handle,
  ...
  return ext4_journal_get_write_access(handle, inode->i_sb, bh, EXT4_JTR_NONE);
}

int ext4_block_write_begin(handle_t* handle,
                           struct folio* folio,
                           loff_t pos,
                           unsigned len,
    ...
    if (!buffer_mapped(bh)) {
      ...
      if (buffer_new(bh)) {
        ...
        if (block_end > to || block_start < from)
          folio_zero_segments(folio, to, block_end, block_start, from);
        continue;
      }
    }
    if (folio_uptodate) {
...
static int ext4_write_begin(const struct kiocb* iocb,
  ...
  *fsdata = (void*)((unsigned long)*fsdata & ~EXT4_WRITE_DATA_INLINE);
   ...
   */
  needed_blocks =
      ext4_chunk_trans_extent(inode, ext4_journal_blocks_per_folio(inode)) + 1;
...
retry_journal:
  ...
  if (!ret && ext4_should_journal_data(inode)) {
    ret = ext4_walk_page_buffers(handle, inode, folio_buffers(folio), from, to,
                                 NULL, do_journal_get_write_access);
  }

  if (ret) {
    bool extended =
        (pos + len > inode->i_size) && !ext4_verity_in_progress(inode);
...
static int write_end_fn(handle_t* handle,
                        struct inode* inode,
                        struct buffer_head* bh) {
  int ret;
  if (!buffer_mapped(bh) || buffer_freed(bh))
...
static int ext4_write_end(const struct kiocb* iocb,
  ...
  if ((unsigned long)fsdata & EXT4_WRITE_DATA_INLINE)
    return ext4_write_inline_data_end(inode, pos, len, copied, folio);
...
static int ext4_journalled_write_end(const struct kiocb* iocb,
  ...
  if (unlikely(copied < len) && !folio_test_uptodate(folio)) {
    ...
    if (unlikely(copied < len))
      ext4_journalled_zero_new_buffers(handle, inode, folio, from + copied, to);
    ret = ext4_walk_page_buffers(handle, inode, folio_buffers(folio), from,
...
static void ext4_print_free_blocks(struct inode* inode) {
  ...
  ext4_msg(sb, KERN_CRIT, "free_blocks=%lld",
           (long long)EXT4_C2B(
               EXT4_SB(sb), percpu_counter_sum(&sbi->s_freeclusters_counter)));
  ext4_msg(sb, KERN_CRIT, "dirty_blocks=%lld",
           (long long)EXT4_C2B(
               EXT4_SB(sb), percpu_counter_sum(&sbi->s_dirtyclusters_counter)));
 ...
 */
static int ext4_insert_delayed_blocks(struct inode* inode,
                                      ext4_lblk_t lblk,
                                      ext4_lblk_t len) {
  struct ext4_sb_info* sbi = EXT4_SB(inode->i_sb);
  int ret;
 ...
 */
static int ext4_da_map_blocks(struct inode* inode,
                              struct ext4_map_blocks* map) {
  struct extent_status es;
  int retval;
#ifdef ES_AGGRESSIVE_TEST
  ...
  if (ext4_es_lookup_extent(inode, map->m_lblk, NULL, &es, NULL)) {
    map->m_len =
        min_t(unsigned int, map->m_len, es.es_len - (map->m_lblk - es.es_lblk));
...
static void mpage_submit_folio(struct mpage_da_data* mpd, struct folio* folio) {
  ...
  if (folio_pos(folio) + len > size && !ext4_verity_in_progress(mpd->inode))
    len = size & (len - 1);
 ...
 * added.
 */
static bool mpage_add_bh_to_extent(struct mpage_da_data* mpd,
                                   ext4_lblk_t lblk,
                                   struct buffer_head* bh) {
 ...
 */
static int mpage_process_folio(struct mpage_da_data* mpd,
                               struct folio* folio,
                               ext4_lblk_t* m_lblk,
                               ext4_fsblk_t* m_pblk,
                               bool* map_bh) {
  ...
  do {
    ...
    if (lblk >= mpd->map.m_lblk + mpd->map.m_len) {
      ...
      if (!err && mpd->map.m_len && mpd->map.m_lblk > lblk) {
        ...
        io_end_vec->offset = EXT4_LBLK_TO_B(mpd->inode, mpd->map.m_lblk);
      }
      *map_bh = true;
      goto out;
    }
...
static int mpage_map_and_submit_buffers(struct mpage_da_data* mpd) {
  ...
  while (start <= end) {
    ...
    for (i = 0; i < nr; i++) {
      ...
      err = mpage_process_folio(mpd, folio, &lblk, &pblock, &map_bh);
      /*
...
static int mpage_map_one_extent(handle_t* handle, struct mpage_da_data* mpd) {
   ...
   */
  get_blocks_flags = EXT4_GET_BLOCKS_CREATE | EXT4_GET_BLOCKS_METADATA_NOFAIL |
...
update_disksize:
  ...
  if (disksize > READ_ONCE(EXT4_I(inode)->i_disksize)) {
    ...
    if (err2) {
      ext4_error_err(inode->i_sb, -err2, "Failed to mark inode %llu dirty",
                     inode->i_ino);
    }
    if (!err)
      err = err2;
...
static int ext4_journal_folio_buffers(handle_t* handle,
                                      struct folio* folio,
                                      size_t len) {
  ...
  ret = ext4_walk_page_buffers(handle, inode, page_bufs, 0, len, NULL,
                               do_journal_get_write_access);
  err = ext4_walk_page_buffers(handle, inode, page_bufs, 0, len, NULL,
                               write_end_fn);
  if (ret == 0)
    ret = err;
...
static int mpage_journal_page_buffers(handle_t* handle,
  ...
  if (folio_pos(folio) + len > size && !ext4_verity_in_progress(inode))
    len = size & (len - 1);
...
static int mpage_prepare_extent_to_map(struct mpage_da_data* mpd) {
  ...
  if (ext4_should_journal_data(mpd->inode)) {
    handle = ext4_journal_start(mpd->inode, EXT4_HT_WRITE_PAGE, bpp);
    if (IS_ERR(handle))
      return PTR_ERR(handle);
  }
  folio_batch_init(&fbatch);
  while (index <= end) {
    ...
    for (i = 0; i < nr_folios; i++) {
      ...
      if (mpd->wbc->sync_mode == WB_SYNC_NONE &&
          mpd->wbc->nr_to_write <= EXT4_LBLK_TO_PG(mpd->inode, mpd->map.m_len))
        goto out;

      /* If we can't merge this page, we are done. */
      if (mpd->map.m_len > 0 && mpd->next_pos != folio_pos(folio))
        goto out;

      if (handle) {
        err = ext4_journal_ensure_credits(handle, bpp, 0);
        if (err < 0)
          goto out;
      }
      ...
      if (!mpd->can_map) {
        ...
        if (folio_test_checked(folio)) {
          err = mpage_journal_page_buffers(handle, mpd, folio);
          if (err < 0)
            goto out;
...
static int ext4_do_writepages(struct mpage_da_data* mpd) {
  ...
  if (ext4_should_journal_data(inode)) {
    ...
    if (wbc->sync_mode == WB_SYNC_ALL)
      ext4_fc_commit(sbi->s_journal, EXT4_I(inode)->i_datasync_tid);
  }
...
retry:
  ...
  while (!mpd->scanned_until_end && wbc->nr_to_write > 0) {
     ...
     */
    needed_blocks = ext4_chunk_trans_blocks(inode, MAX_WRITEPAGES_EXTENT_LEN);
    /* start a new transaction */
    handle = ext4_journal_start_with_reserve(inode, EXT4_HT_WRITE_PAGE,
                                             needed_blocks, rsv_blocks);
    if (IS_ERR(handle)) {
      ret = PTR_ERR(handle);
      ext4_msg(inode->i_sb, KERN_CRIT,
               "%s: jbd2_start: "
               "%ld pages, ino %llu; err %d",
    ...
    trace_ext4_da_write_folios_start(inode, mpd->start_pos, mpd->next_pos, wbc);
    ret = mpage_prepare_extent_to_map(mpd);
    ...
    if (handle) {
      ...
      ext4_put_io_end(mpd->io_submit.io_end);
    mpd->io_submit.io_end = NULL;
    trace_ext4_da_write_folios_end(inode, mpd->start_pos, mpd->next_pos, wbc,
                                   ret);
...
static int ext4_dax_writepages(struct address_space* mapping,
  ...
  ret =
      dax_writeback_mapping_range(mapping, EXT4_SB(inode->i_sb)->s_daxdev, wbc);
  trace_ext4_writepages_result(inode, wbc, ret, nr_to_write - wbc->nr_to_write);
...
static int ext4_da_write_begin(const struct kiocb* iocb,
  ...
  if (ext4_nonda_switch(inode->i_sb) || ext4_verity_in_progress(inode)) {
    *fsdata = (void*)FALL_BACK_TO_NONDELALLOC;
    return ext4_write_begin(iocb, mapping, pos, len, foliop, fsdata);
  }
  *fsdata = (void*)0;
  ...
  if (ext4_test_inode_state(inode, EXT4_STATE_MAY_INLINE_DATA)) {
    ret =
        ext4_generic_write_inline_data(mapping, inode, pos, len, foliop, true);
    if (ret < 0)
      return ret;
    if (ret == 1) {
      *fsdata = (void*)((unsigned long)*fsdata | EXT4_WRITE_DATA_INLINE);
...
retry:
  ...
  ret = ext4_block_write_begin(NULL, folio, pos, len, ext4_da_get_block_prep);
  if (ret < 0) {
    folio_unlock(folio);
    folio_put(folio);
    /*
...
static int ext4_da_write_end(const struct kiocb* iocb,
  ...
  if (write_mode & FALL_BACK_TO_NONDELALLOC)
    return ext4_write_end(iocb, mapping, pos, len, copied, folio, fsdata);
...
static sector_t ext4_bmap(struct address_space* mapping, sector_t block) {
  ...
  if (mapping_tagged(mapping, PAGECACHE_TAG_DIRTY) &&
      (test_opt(inode->i_sb, DELALLOC) || ext4_should_journal_data(inode))) {
    /*
...
static void ext4_invalidate_folio(struct folio* folio,
                                  size_t offset,
                                  size_t length) {
...
static void ext4_set_iomap(struct inode* inode,
                           struct iomap* iomap,
                           struct ext4_map_blocks* map,
                           loff_t offset,
                           loff_t length,
   ...
   */
  iomap->flags = 0;
  if (ext4_inode_datasync_dirty(inode) || offset + length > i_size_read(inode))
...
static int ext4_map_blocks_atomic_write_slow(handle_t* handle,
  ...
  do {
     ...
     * physical extents.
     */
    if ((check_next_pblk && next_pblk != map->m_pblk) || ret == -ENOSPC) {
      ext4_warning_inode(
          inode,
  ...
  ret = ext4_map_blocks(handle, inode, map, EXT4_GET_BLOCKS_QUERY_LAST_IN_LEAF);
  if (ret != m_len) {
    ext4_warning_inode(inode,
                       "allocation failed for atomic write request m_lblk:%u, "
                       "m_len:%u, ret:%d\n",
                       m_lblk, m_len, ret);
    ret = -EINVAL;
  }
 ...
 */
static int ext4_map_blocks_atomic_write(handle_t* handle,
                                        struct inode* inode,
                                        struct ext4_map_blocks* map,
                                        int m_flags,
                                        bool* force_commit) {
...
static int ext4_iomap_alloc(struct inode* inode,
  ...
  if (flags & IOMAP_ATOMIC) {
    ...
    if (map->m_len < orig_mlen) {
      map->m_len = orig_mlen;
      dio_credits = ext4_meta_trans_blocks(inode, map->m_len, map->m_len, 0);
    } else {
...
int ext4_iomap_begin(struct inode* inode,
  ...
  map.m_len =
      min_t(loff_t, (offset + length - 1) >> blkbits, EXT4_MAX_LOGICAL_BLOCK) -
      map.m_lblk + 1;
  orig_mlen = map.m_len;
  ...
  if (flags & IOMAP_WRITE) {
    ...
    if (offset + length <= i_size_read(inode)) {
       ...
       */
      if ((map.m_flags & EXT4_MAP_MAPPED) ||
          (!(flags & IOMAP_DAX) && (map.m_flags & EXT4_MAP_UNWRITTEN))) {
        /*
         * For atomic writes the entire requested
         * length should be mapped.
         */
        if (ret == orig_mlen || (!(flags & IOMAP_ATOMIC) && ret > 0))
          goto out;
      }
      map.m_len = orig_mlen;
    }
...
static int ext4_iomap_begin_report(struct inode* inode,
                                   loff_t offset,
                                   loff_t length,
                                   unsigned int flags,
                                   struct iomap* iomap,
...
static int ext4_block_do_zero_range(struct inode* inode,
                                    loff_t from,
                                    loff_t length,
                                    bool* did_zero,
                                    bool* zero_written) {
...
static int ext4_block_journalled_zero_range(struct inode* inode,
                                            loff_t from,
                                            loff_t length,
                                            bool* did_zero) {
  struct buffer_head* bh;
...
static int ext4_block_zero_range(struct inode* inode,
  ...
  if (IS_DAX(inode)) {
    return dax_zero_range(inode, from, length, did_zero, &ext4_iomap_ops);
  } else if (ext4_should_journal_data(inode)) {
    return ext4_block_journalled_zero_range(inode, from, length, did_zero);
  }
  return ext4_block_do_zero_range(inode, from, length, did_zero, zero_written);
}
...
int ext4_block_zero_eof(struct inode* inode, loff_t from, loff_t end) {
  ...
  err = ext4_block_zero_range(inode, from, length, &did_zero, &zero_written);
  if (err)
    return err;
  /*
   ...
   */
  if (ext4_should_order_data(inode) && did_zero && zero_written &&
      !IS_DAX(inode)) {
    handle_t* handle;
 ...
 */
int ext4_zero_partial_blocks(struct inode* inode,
                             loff_t lstart,
                             loff_t length,
  ...
  if (start == end && (partial_start || (partial_end != sb->s_blocksize - 1))) {
    err = ext4_block_zero_range(inode, lstart, length, &did_zero, NULL);
    if (did_zero)
      *partial_zeroed |= (EXT4_PARTIAL_ZERO_START | EXT4_PARTIAL_ZERO_END);
    return err;
  }
  /* Handle partial zero out on the start of the range */
  if (partial_start) {
    err =
        ext4_block_zero_range(inode, lstart, sb->s_blocksize, &did_zero, NULL);
    if (err)
      return err;
    if (did_zero)
  ...
  if (partial_end != sb->s_blocksize - 1) {
    did_zero = false;
    err = ext4_block_zero_range(inode, byte_end - partial_end, partial_end + 1,
                                &did_zero, NULL);
    if (err)
      return err;
    if (did_zero)
...
int ext4_truncate_page_cache_block_range(struct inode* inode,
   ...
   */
  if (!IS_ALIGNED(start | end, PAGE_SIZE) && blocksize < PAGE_SIZE &&
      start < inode->i_size) {
...
int ext4_punch_hole(struct file* file, loff_t offset, loff_t length) {
  ...
  if (end_lblk > start_lblk) {
    ...
    if (ext4_test_inode_flag(inode, EXT4_INODE_EXTENTS))
      ...
      ret = ext4_ind_remove_space(handle, inode, start_lblk, end_lblk);
    if (ret) {
      up_write(&EXT4_I(inode)->i_data_sem);
      goto out_handle;
    }

    ext4_es_insert_extent(inode, start_lblk, hole_len, ~0, EXTENT_STATUS_HOLE,
                          0);
    up_write(&EXT4_I(inode)->i_data_sem);
  }
...
static int ext4_fill_raw_inode(struct inode* inode,
  ...
  if (i_projid != EXT4_DEF_PROJID && !ext4_has_feature_project(inode->i_sb))
...
static int __ext4_get_inode_loc_noinmem(struct inode* inode,
  ...
  ret = __ext4_get_inode_loc(inode->i_sb, inode->i_ino, NULL, iloc, &err_blk);

  if (ret == -EIO)
    ext4_error_inode_block(inode, err_blk, EIO, "unable to read itable block");
...
int ext4_get_inode_loc(struct inode* inode, struct ext4_iloc* iloc) {
  ...
  ret = __ext4_get_inode_loc(inode->i_sb, inode->i_ino, inode, iloc, &err_blk);
...
int ext4_get_inode_loc_noio(struct inode* inode, struct ext4_iloc* iloc) {
  ...
  if (block <= le32_to_cpu(EXT4_SB(sb)->s_es->s_first_data_block) ||
      block >= ext4_blocks_count(EXT4_SB(sb)->s_es)) {
    ext4_error(sb, "Invalid inode table block %llu in block_group %u", block,
               iloc->block_group);
    return -EFSCORRUPTED;
  }
  block += inode_offset / inodes_per_block;
...
void ext4_set_inode_flags(struct inode* inode, bool init) {
  ...
  inode_set_flags(inode, new_fl,
                  S_SYNC | S_APPEND | S_IMMUTABLE | S_NOATIME | S_DIRSYNC |
                      S_DAX | S_ENCRYPTED | S_CASEFOLD | S_VERITY);
}
...
static inline int ext4_iget_extra_inode(struct inode* inode,
                                        ...
                                        struct ext4_inode_info* ei) {
  __le32* magic =
      (void*)raw_inode + EXT4_GOOD_OLD_INODE_SIZE + ei->i_extra_isize;
...
static int check_igot_inode(struct inode* inode,
...
struct inode* __ext4_iget(struct super_block* sb,
                          unsigned long ino,
                          ext4_iget_flags flags,
                          const char* function,
                          unsigned int line) {
  ...
  if ((!(flags & EXT4_IGET_SPECIAL) && is_special_ino(sb, ino)) ||
      (ino < EXT4_ROOT_INO) || (ino > le32_to_cpu(es->s_inodes_count))) {
    if (flags & EXT4_IGET_HANDLE)
  ...
  if (size < 0 || size > ext4_get_maxbytes(inode)) {
    ext4_error_inode(inode, function, line, 0, "iget: bad i_size value: %lld",
                     size);
    ret = -EFSCORRUPTED;
    goto bad_inode;
  }
   ...
   */
  if (!ext4_has_feature_dir_index(sb) && ext4_has_feature_metadata_csum(sb) &&
  ...
  ret = 0;
  if (ei->i_file_acl && !ext4_inode_block_valid(inode, ei->i_file_acl, 1)) {
    ext4_error_inode(inode, function, line, 0,
                     "iget: bad extended attribute block %llu", ei->i_file_acl);
    ret = -EFSCORRUPTED;
    goto bad_inode;
  } else if (!ext4_has_inline_data(inode)) {
  ...
  if (S_ISREG(inode->i_mode)) {
    ...
    if (IS_ENCRYPTED(inode)) {
      ...
      if ((EXT4_SB(sb)->s_mount_state & EXT4_ORPHAN_FS)) {
        if (inode->i_nlink != 0) {
          ext4_error_inode(inode, function, line, 0,
                           "invalid orphan symlink nlink %d", inode->i_nlink);
          ret = -EFSCORRUPTED;
          goto bad_inode;
        }
      } else {
        if (inode->i_size == 0 || inode->i_size >= sizeof(ei->i_data) ||
            strnlen((char*)ei->i_data, inode->i_size + 1) != inode->i_size) {
          ext4_error_inode(inode, function, line, 0,
        ...
        inode_set_cached_link(inode, (char*)ei->i_data, inode->i_size);
      }
    } else {
    ...
    ret = -EFSCORRUPTED;
    ext4_error_inode(inode, function, line, 0, "iget: bogus i_mode (%o)",
                     inode->i_mode);
    goto bad_inode;
  }
...
static void ext4_update_other_inodes_time(struct super_block* sb,
  ...
  for (i = 0; i < inodes_per_block; i++, ino++, buf += inode_size) {
    if (ino == orig_ino)
      continue;
    __ext4_update_other_inode_time(sb, orig_ino, ino, (struct ext4_inode*)buf);
  }
  rcu_read_unlock();
}
...
int ext4_sync_inode_metadata(struct inode* inode,
                             struct writeback_control* wbc) {
  struct ext4_iloc iloc;
  struct mapping_metadata_bhs* mmb;
  int err;
 ...
 * invalidate buffers that are attached to a folio straddling i_size and are
 * undergoing commit. In that case we have to wait for commit to finish and try
 * again.
 */
static void ext4_wait_for_tail_page_commit(struct inode* inode) {
  ...
  while (1) {
    struct folio* folio =
        filemap_lock_folio(inode->i_mapping, inode->i_size >> PAGE_SHIFT);
    if (IS_ERR(folio))
      return;
 ...
 */
int ext4_setattr(struct mnt_idmap* idmap,
  ...
  if (unlikely(IS_APPEND(inode) &&
               (ia_valid & (ATTR_MODE | ATTR_UID | ATTR_GID | ATTR_TIMES_SET))))
    return -EPERM;
    ...
    if (attr->ia_size != inode->i_size) {
      ...
      if (!shrink) {
        ...
        if (oldsize & (inode->i_sb->s_blocksize - 1)) {
          error = ext4_block_zero_eof(inode, oldsize, LLONG_MAX);
          if (error)
            goto out_mmap_sem;
        }
      }
...
int ext4_getattr(struct mnt_idmap* idmap,
...
int ext4_file_getattr(struct mnt_idmap* idmap,
   ...
   */
  delalloc_blocks =
      EXT4_C2B(EXT4_SB(inode->i_sb), EXT4_I(inode)->i_reserved_data_blocks);
  stat->blocks += delalloc_blocks << (inode->i_sb->s_blocksize_bits - 9);
  return 0;
}

static int ext4_index_trans_blocks(struct inode* inode,
                                   int lblocks,
                                   int pextents) {
  if (!(ext4_test_inode_flag(inode, EXT4_INODE_EXTENTS)))
...
int ext4_reserve_inode_write(handle_t* handle,
  ...
  if (!err) {
    ...
    err = ext4_journal_get_write_access(handle, inode->i_sb, iloc->bh,
                                        EXT4_JTR_NONE);
    if (err) {
      brelse(iloc->bh);
      iloc->bh = NULL;
...
static int ext4_try_to_expand_extra_isize(struct inode* inode,
   ...
   */
  if (ext4_journal_extend(handle, EXT4_DATA_TRANS_BLOCKS(inode->i_sb), 0) != 0)
    return -ENOSPC;
...
int __ext4_mark_inode_dirty(handle_t* handle,
  ...
  if (EXT4_I(inode)->i_extra_isize < sbi->s_want_extra_isize)
    ext4_try_to_expand_extra_isize(inode, sbi->s_want_extra_isize, iloc,
                                   handle);

  err = ext4_mark_iloc_dirty(handle, inode, &iloc);
out:
  if (unlikely(err))
    ext4_error_inode_err(inode, func, line, 0, err, "mark_inode_dirty error");
  return err;
}
...
int ext4_change_inode_journal_flag(struct inode* inode, int val) {
  ...
  ext4_fc_mark_ineligible(inode->i_sb, EXT4_FC_REASON_JOURNAL_FLAG_CHANGE,
                          handle);
  err = ext4_mark_inode_dirty(handle, inode);
  ext4_handle_sync(handle);
  ext4_journal_stop(handle);
...
static int ext4_bh_unmapped(handle_t* handle,
                            struct inode* inode,
                            struct buffer_head* bh) {
  return !buffer_mapped(bh);
}

static int ext4_block_page_mkwrite(struct inode* inode,
                                   struct folio* folio,
                                   get_block_t get_block) {
  handle_t* handle;
  loff_t size;
  ...
  credits =
      ext4_chunk_trans_extent(inode, ext4_journal_blocks_per_folio(inode));
...
vm_fault_t ext4_page_mkwrite(struct vm_fault* vmf) {
  ...
  if (test_opt(inode->i_sb, DELALLOC) && !ext4_nonda_switch(inode->i_sb)) {
    do {
      err = block_page_mkwrite(vma, vmf, ext4_da_get_block_prep);
    } while (err == -ENOSPC && ext4_should_retry_alloc(inode->i_sb, &retries));
    goto out_ret;
  }
   ...
   */
  if (folio_buffers(folio)) {
    if (!ext4_walk_page_buffers(NULL, inode, folio_buffers(folio), 0, len, NULL,
                                ext4_bh_unmapped)) {
