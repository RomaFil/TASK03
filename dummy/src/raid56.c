...
static void dump_bioc(const struct btrfs_fs_info* fs_info,
  ...
  btrfs_crit(
      fs_info,
      "bioc logical=%llu full_stripe=%llu size=%llu map_type=0x%llx mirror=%u "
      "replace_nr_stripes=%u replace_stripe_src=%d num_stripes=%u",
      bioc->logical, bioc->full_stripe_logical, bioc->size, bioc->map_type,
      bioc->mirror_num, bioc->replace_nr_stripes, bioc->replace_stripe_src,
      bioc->num_stripes);
  for (int i = 0; i < bioc->num_stripes; i++) {
    btrfs_crit(fs_info, "    nr=%d devid=%llu physical=%llu", i,
               bioc->stripes[i].dev->devid, bioc->stripes[i].physical);
  }
}
...
static void btrfs_dump_rbio(const struct btrfs_fs_info* fs_info,
  ...
  btrfs_crit(fs_info,
             "rbio flags=0x%lx nr_sectors=%u nr_data=%u real_stripes=%u "
             "stripe_nsectors=%u sector_nsteps=%u scrubp=%u dbitmap=0x%lx",
             rbio->flags, rbio->nr_sectors, rbio->nr_data, rbio->real_stripes,
             rbio->stripe_nsectors, rbio->sector_nsteps, rbio->scrubp,
             rbio->dbitmap);
}
...
static void memcpy_from_bio_to_stripe(struct btrfs_raid_bio* rbio,
  ...
  for (int i = 0; i < rbio->sector_nsteps; i++) {
    ...
    memcpy_page(phys_to_page(dst), offset_in_page(dst), phys_to_page(src),
                offset_in_page(src), step);
  }
}
...
static u32 page_nr_to_num_sectors(struct btrfs_raid_bio* rbio,
  ...
  nr_sectors =
      round_up(PAGE_SIZE, fs_info->sectorsize) >> fs_info->sectorsize_bits;
  ASSERT(nr_sectors > 0);
  return nr_sectors;
}

static __maybe_unused bool full_page_sectors_uptodate(
...
static void index_stripe_sectors(struct btrfs_raid_bio* rbio) {
  ...
  for (i = 0, offset = 0; i < rbio->nr_sectors * rbio->sector_nsteps;
    ...
    rbio->stripe_paddrs[i] =
        page_to_phys(rbio->stripe_pages[page_index]) + offset_in_page(offset);
  }
}
...
static void btrfs_clear_rbio_cache(struct btrfs_fs_info* info) {
  ...
  while (!list_empty(&table->stripe_cache)) {
    rbio = list_first_entry(&table->stripe_cache, struct btrfs_raid_bio,
                            stripe_cache);
    __remove_rbio_from_cache(rbio);
  }
  spin_unlock(&table->cache_lock);
...
static unsigned int rbio_paddr_index(const struct btrfs_raid_bio* rbio,
  ...
  ret = rbio_sector_index(rbio, stripe_nr, sector_nr) * rbio->sector_nsteps +
        step_nr;
...
static phys_addr_t rbio_stripe_paddr(const struct btrfs_raid_bio* rbio,
                                     ...
                                     unsigned int step_nr) {
  return rbio
      ->stripe_paddrs[rbio_paddr_index(rbio, stripe_nr, sector_nr, step_nr)];
}
...
/* Return a paddr pointer into the rbio::stripe_paddrs[] for the specified
 * sector. */
...
static noinline int lock_stripe_add(struct btrfs_raid_bio* rbio) {
  ...
  list_for_each_entry(cur, &h->hash_list, hash_list) {
    ...
    if (bio_list_empty(&cur->bio_list) && list_empty(&cur->plug_list) &&
        test_bit(RBIO_CACHE_BIT, &cur->flags) &&
        !test_bit(RBIO_RMW_LOCKED_BIT, &cur->flags)) {
...
static noinline void unlock_stripe(struct btrfs_raid_bio* rbio) {
  ...
  if (!list_empty(&rbio->hash_list)) {
    ...
    if (!list_empty(&rbio->plug_list)) {
      ...
      next = list_entry(head, struct btrfs_raid_bio, plug_list);
...
static struct btrfs_raid_bio* alloc_rbio(struct btrfs_fs_info* fs_info,
  ...
  rbio->bio_paddrs =
      kzalloc_objs(phys_addr_t, num_sectors * sector_nsteps, GFP_NOFS);
  rbio->stripe_paddrs =
      kzalloc_objs(phys_addr_t, num_sectors * sector_nsteps, GFP_NOFS);
 ...
 */
static int get_rbio_vertical_errors(struct btrfs_raid_bio* rbio,
                                    int sector_nr,
                                    int* faila,
                                    int* failb) {
  int stripe_nr;
...
static int bio_add_paddrs(struct bio* bio,
                          phys_addr_t* paddrs,
...
static int rbio_add_io_paddrs(struct btrfs_raid_bio* rbio,
  ...
  ASSERT_RBIO_STRIPE(stripe_nr >= 0 && stripe_nr < rbio->bioc->num_stripes,
                     rbio, stripe_nr);
  ASSERT_RBIO_SECTOR(sector_nr >= 0 && sector_nr < rbio->stripe_nsectors, rbio,
                     sector_nr);
  ASSERT(paddrs != NULL);
  ...
  if (!stripe->dev->bdev) {
    ...
    set_bit(stripe_nr * rbio->stripe_nsectors + sector_nr, rbio->error_bitmap);
  ...
  bio = bio_alloc(stripe->dev->bdev, max(BTRFS_STRIPE_LEN >> PAGE_SHIFT, 1), op,
                  GFP_NOFS);
...
static void index_rbio_pages(struct btrfs_raid_bio* rbio) {
  ...
  spin_lock(&rbio->bio_list_lock);
  bio_list_for_each(bio, &rbio->bio_list) index_one_bio(rbio, bio);

  spin_unlock(&rbio->bio_list_lock);
}

static void bio_get_trace_info(struct btrfs_raid_bio* rbio,
                               struct bio* bio,
  ...
  for (i = 0; i < bioc->num_stripes; i++) {
    ...
    trace_info->offset =
        (bio->bi_iter.bi_sector << SECTOR_SHIFT) - bioc->stripes[i].physical;
    return;
  }
...
static void generate_pq_vertical_step(struct btrfs_raid_bio* rbio,
  ...
  if (has_qstripe) {
     ...
     */
    pointers[stripe++] =
        kmap_local_paddr(rbio_qstripe_paddr(rbio, sector_nr, step_nr));
    ...
    xor_gen(pointers[rbio->nr_data], pointers + 1, rbio->nr_data - 1, step);
  }
...
static int rmw_assemble_write_bios(struct btrfs_raid_bio* rbio,
  ...
  for (total_sector_nr = 0; total_sector_nr < rbio->nr_sectors;
    ...
    ret = rbio_add_io_paddrs(rbio, bio_list, paddrs, stripe, sectornr,
                             REQ_OP_WRITE);
    if (ret)
      goto error;
  }
  ...
  if (bio->bi_iter.bi_size == 0) {
    ...
    for (stripe_nr = 0; stripe_nr < rbio->real_stripes; stripe_nr++) {
      if (!rbio->bioc->stripes[stripe_nr].dev->bdev) {
        found_missing = true;
        bitmap_set(rbio->error_bitmap, stripe_nr * rbio->stripe_nsectors,
...
static void rbio_update_error_bitmap(struct btrfs_raid_bio* rbio,
  ...
  for (int i = total_sector_nr;
       i < total_sector_nr + (bio_size >> rbio->bioc->fs_info->sectorsize_bits);
       i++)
...
static void rbio_add_bio(struct btrfs_raid_bio* rbio, struct bio* orig_bio) {
  ...
  ASSERT_RBIO_LOGICAL(
      orig_logical >= full_stripe_start &&
          orig_logical + orig_len <=
              full_stripe_start + rbio->nr_data * BTRFS_STRIPE_LEN,
      rbio, orig_logical);
  ...
  for (cur_logical = orig_logical; cur_logical < orig_logical + orig_len;
    ...
    int bit =
        ((u32)(cur_logical - full_stripe_start) >> fs_info->sectorsize_bits) %
        rbio->stripe_nsectors;
...
static int verify_one_sector(struct btrfs_raid_bio* rbio,
  ...
  csum_expected =
      rbio->csum_buf +
      (stripe_nr * rbio->stripe_nsectors + sector_nr) * fs_info->csum_size;
  btrfs_calculate_block_csum_pages(fs_info, paddrs, csum_buf);
...
static void recover_vertical_step(struct btrfs_raid_bio* rbio,
  ...
  if (rbio->bioc->map_type & BTRFS_BLOCK_GROUP_RAID6) {
    ...
    if (failb == rbio->real_stripes - 2) {
      raid6_recov_datap(rbio->real_stripes, step, faila, pointers);
    } else {
      raid6_recov_2data(rbio->real_stripes, step, faila, failb, pointers);
    }
  } else {
    void* p;
  ...
  pstripe:
    ...
    p = pointers[faila];
    for (stripe_nr = faila; stripe_nr < rbio->nr_data - 1; stripe_nr++)
 ...
 */
static int recover_vertical(struct btrfs_raid_bio* rbio,
                            int sector_nr,
                            void** pointers,
                            void** unmap_array) {
  int found_errors;
  ...
  for (int i = 0; i < rbio->sector_nsteps; i++)
    recover_vertical_step(rbio, sector_nr, i, faila, failb, pointers,
                          unmap_array);
  if (faila >= 0) {
...
static void set_rbio_raid6_extra_error(struct btrfs_raid_bio* rbio,
  ...
  for (sector_nr = 0; sector_nr < rbio->stripe_nsectors; sector_nr++) {
    ...
    if (failb >= 0)
      set_bit(failb * rbio->stripe_nsectors + sector_nr, rbio->error_bitmap);
  }
 ...
 * of the drive.
 */
void raid56_parity_recover(struct bio* bio,
                           struct btrfs_io_context* bioc,
                           int mirror_num) {
  struct btrfs_fs_info* fs_info = bioc->fs_info;
  struct btrfs_raid_bio* rbio;
...
static void fill_data_csums(struct btrfs_raid_bio* rbio) {
  ...
  rbio->csum_buf = kzalloc(
      rbio->nr_data * rbio->stripe_nsectors * fs_info->csum_size, GFP_NOFS);
  rbio->csum_bitmap =
  ...
  if (unlikely(!csum_root)) {
    btrfs_err(fs_info, "missing csum root for extent at bytenr %llu",
              rbio->bioc->full_stripe_logical);
    ret = -EUCLEAN;
    goto error;
  }
...
error:
  ...
  btrfs_warn_rl(fs_info,
                "sub-stripe write for full stripe %llu is not safe, failed to "
                "get csum: %d",
...
static bool need_read_stripe_sectors(struct btrfs_raid_bio* rbio) {
  ...
  for (i = 0; i < rbio->nr_data * rbio->stripe_nsectors; i++) {
     ...
     */
    if (paddr == INVALID_PADDR || !test_bit(i, rbio->stripe_uptodate_bitmap))
      return true;
  }
  return false;
}
...
struct btrfs_raid_bio* raid56_parity_alloc_scrub_rbio(
    struct bio* bio,
    struct btrfs_io_context* bioc,
...
static int finish_parity_scrub(struct btrfs_raid_bio* rbio) {
  ...
  for_each_set_bit(sectornr, &rbio->dbitmap, rbio->stripe_nsectors) {
    ...
    ret = rbio_add_io_paddrs(rbio, &bio_list, paddrs, rbio->scrubp, sectornr,
                             REQ_OP_WRITE);
    if (ret)
      goto cleanup;
  }
