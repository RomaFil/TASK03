 ...
 */
static int mraid_mm_ioctl(struct file* filep,
                          unsigned int cmd,
                          unsigned long arg) {
  uioc_t* kioc;
  ...
  if (!adp->quiescent) {
    con_log(CL_ANN,
            (KERN_WARNING "megaraid cmm: controller cannot accept cmds due to "
                          "earlier errors\n"));
    return -EFAULT;
  }
...
static long mraid_mm_unlocked_ioctl(struct file* filep,
                                    unsigned int cmd,
                                    unsigned long arg) {
  int err;
  ...
  switch (subopcode) {
    ...
    case MEGAIOC_QNADAP:
      ...
      *rval = adapters_count_g;
      ...
      if (copy_to_user(kmimd.data, &adapters_count_g, sizeof(uint32_t)))
        return (-EFAULT);
...
static int mimd_to_kioc(mimd_t __user* umimd,
                        mraid_mmadp_t* adp,
                        uioc_t* kioc) {
  mbox64_t* mbox64;
  mbox_t* mbox;
  ...
  switch (opcode) {
    case 0x82:
      ...
      if (subopcode == MEGAIOC_QADAPINFO) {
        ...
        con_log(CL_ANN, (KERN_WARNING "megaraid cmm: Invalid subop\n"));
        return (-EINVAL);
      }
  ...
  if (mbox->cmd != MBOXCMD_PASSTHRU) {  // regular DCMD
    ...
    if (kioc->data_dir & UIOC_WR) {
      if (copy_from_user(kioc->buf_vaddr, kioc->user_data, kioc->xferlen)) {
        return (-EFAULT);
      }
    }
  ...
  if (copy_from_user(pthru32, kioc->user_pthru, sizeof(mraid_passthru_t))) {
    return (-EFAULT);
  }
...
static int mraid_mm_attach_buf(mraid_mmadp_t* adp, uioc_t* kioc, int xferlen) {
  ...
  kioc->free_buf = 1;
  kioc->buf_vaddr = dma_pool_alloc(pool->handle, GFP_ATOMIC, &kioc->buf_paddr);
  spin_unlock_irqrestore(&pool->lock, flags);
...
static int kioc_to_mimd(uioc_t* kioc, mimd_t __user* mimd) {
  ...
  if (opcode == 0x82) {
    switch (subopcode) {
      case MEGAIOC_QADAPINFO:
        ...
        hinfo = (mraid_hba_info_t*)(unsigned long)kioc->buf_vaddr;
  ...
  if (kioc->user_pthru) {
    ...
    if (copy_to_user(&upthru32->scsistatus, &kpthru32->scsistatus,
                     sizeof(uint8_t))) {
      return (-EFAULT);
    }
  }
  ...
  if (kioc->user_data) {
    if (copy_to_user(kioc->user_data, kioc->buf_vaddr, kioc->user_data_len)) {
      return (-EFAULT);
    }
  }

  if (copy_to_user(&mimd->mbox[17], &mbox64->mbox32.status, sizeof(uint8_t))) {
    return (-EFAULT);
  }
 ...
 */
int mraid_mm_register_adp(mraid_mmadp_t* adapter) {
  mbox64_t* mbox_list;
  uioc_t* kioc;
  uint32_t rval;
  int i;
  ...
  adapter->pthru_dma_pool =
      dma_pool_create("megaraid mm pthru pool", &adapter->pdev->dev,
                      sizeof(mraid_passthru_t), 16, 0);

  if (!adapter->kioc_list || !adapter->mbox_list || !adapter->pthru_dma_pool) {
  ...
  for (i = 0; i < adapter->max_kioc; i++) {
    ...
    kioc->pthru32 =
        dma_pool_alloc(adapter->pthru_dma_pool, GFP_KERNEL, &kioc->pthru32_h);
...
pthru_dma_pool_error:
  ...
  for (i = 0; i < adapter->max_kioc; i++) {
    kioc = adapter->kioc_list + i;
    if (kioc->pthru32) {
      dma_pool_free(adapter->pthru_dma_pool, kioc->pthru32, kioc->pthru32_h);
    }
  }
...
static int mraid_mm_setup_dma_pools(mraid_mmadp_t* adp) {
  ...
  for (i = 0; i < MAX_DMA_POOLS; i++) {
    ...
    pool->handle = dma_pool_create("megaraid mm data buffer", &adp->pdev->dev,
                                   bufsize, 16, 0);
    ...
    pool->vaddr = dma_pool_alloc(pool->handle, GFP_KERNEL, &pool->paddr);
...
static void mraid_mm_free_adp_resources(mraid_mmadp_t* adp) {
  ...
  for (i = 0; i < adp->max_kioc; i++) {
    ...
    dma_pool_free(adp->pthru_dma_pool, kioc->pthru32, kioc->pthru32_h);
  }
...
static void mraid_mm_teardown_dma_pools(mraid_mmadp_t* adp) {
  ...
  for (i = 0; i < MAX_DMA_POOLS; i++) {
    ...
    if (pool->handle) {
      if (pool->vaddr)
        dma_pool_free(pool->handle, pool->vaddr, pool->paddr);
