 ...
 * and P/Q stripes. For now, each data and P/Q stripe is of a fixed length
 * (64K).
 *
 ...
 * be INVALID_PADDR.
 *
 * The length of each entry in bio_paddrs[] is a step (aka, min(sectorsize,
 * PAGE_SIZE)).
 *
 ...
 * A special value INVALID_PADDR represents when the physical address is
 * invalid, normally meaning there is no page allocated for the specified
 * sector.
 */
struct btrfs_raid_bio {
  ...
  /* Each set bit means the corresponding sector in stripe_sectors[] is
   * uptodate. */
